...
export default function AdminCategory() {
    ...
    const handleAddCategory = async () => {
        ...
        try {
            const res = await fetch(
                `${API_URL}/category/add/${encodeURIComponent(newCategoryName.trim())}`,
                { method: "POST" }
            );
    ...
    const handleUpdateCategory = async (categoryId) => {
        ...
        try {
            const res = await fetch(
                `${API_URL}/category/update-category/${categoryId}?newCategoryName=${encodeURIComponent(
                    editingName.trim()
                )}`,
                { method: "PUT" }
            );
    ...
    const handleDeleteCategory = async () => {
        try {
            const res = await fetch(
                `${API_URL}/category/delete/${deleteCategoryId}`,
                { method: "DELETE" }
            );
            const data = await res.json();
            setSuccessMessage(data.message);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-1 md:p-4">
            <div className="max-w-7xl mx-auto">
                ...
                {/* Top Bar */}
                <div className="flex flex-col md:flex-row gap-2 justify-between items-center mt-6 mb-6">
                    {/* Search */}
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-emerald-500 text-white px-5 py-2 rounded-lg hover:bg-green-700 transition w-full md:w-auto"
                    >
                ...
                <div className="bg-white shadow-md rounded-lg overflow-hidden">
                    <table className="w-full">
                        <thead className="bg-gray-100 border-b">
                            <tr>
                                <th className="text-left px-6 py-3 text-sm font-semibold text-gray-700">No.</th>
                                <th className="text-left px-6 py-3 text-sm font-semibold text-gray-700">Category Name</th>
